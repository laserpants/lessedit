...
static void td_expand_tabs(struct buf *ob, const uint8_t *line, size_t size)
{
    size_t  i = 0, tab = 0;
...
static int
td_parse_block(struct buf *ob, struct sd_markdown *rndr, uint8_t *data, size_t size)
{
    size_t beg, end, i;
    uint8_t *txt_data;
    size_t obs = 0;
    ...
    size_t depth = rndr->work_bufs[BUFFER_SPAN].size + rndr->work_bufs[BUFFER_BLOCK].size;

    if (depth > rndr->max_nesting)
        return 0;
...
int
td_markdown_render(struct buf *ob, const uint8_t *document, size_t doc_size, struct sd_markdown *md)
{
#define MARKDOWN_GROW(x) ((x) + ((x) >> 1))
    static const char UTF8_BOM[] = {0xEF, 0xBB, 0xBF};
    ...
    while (beg < doc_size) /* iterating over lines */
        if (is_ref(document, beg, doc_size, &end, md->refs))
            ...
            /* adding the line body if present */
            if (end > beg)
                td_expand_tabs(text, document + beg, end - beg);
    ...
    if (text->size) {
        ...
        data_size = td_parse_block(ob, md, text->data, text->size);
    }
